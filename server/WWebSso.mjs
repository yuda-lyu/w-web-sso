...
function WWebSso(WOrm, url, db, getUserByToken, verifyBrowserUser, verifyAppUser, opt = {}) {
    ...
    let apis = [
        ...
        {
            method: 'GET',
            path: '/api/getUsersByToken', //名稱getUserByToken為w-ui-loginout預設值, 若要更改兩邊須同時修改
            handler: async function (req, res) {
                // console.log('getUsersByToken', req)
                ...
                //     //getAndVerifyBrowserTokenUser
                //     let userSelf = await getAndVerifyBrowserTokenUser(token, 'getUsersByToken')
                //     // console.log('userSelf', userSelf)
