    ...
    methods: {
        ...
        genOpt: function() {
            ...
            if (size(vo.items) > 0 || !vo.firstLoading) {
                ...
                opt = {
                    ...
                    kpHeadFocusHighlight: { //此四欄之儲存格不顯示焦點框(欄內為按鈕或時間選擇器, 焦點框會與控制項本身之框線疊加); w-aggrid-vue 2.0.87 起補 :focus-within, 子控制項取得焦點時亦確實不顯示(該版前僅 :focus, 故效果不完全)
                        'password': false,
                        'timeVerified': false,
                        'timeExpired': false,
                        'timeBlocked': false,
        ...
        saveUsers: function() {
            ...
            async function core() {
                ...
                //每個 await $dg.showCheckYes(...) 前先關 loading, 避免 modal 等待期 loading spinner 疊著
                //造成 UX 怪. early-return 各自關閉確保所有路徑離開時 loading 已關 (含 sync 檢測 early-return).
                //finally 統一保險. 對應 ADR-002 訊息呈現分流之 showCheckYes modal 持久阻塞語意.
