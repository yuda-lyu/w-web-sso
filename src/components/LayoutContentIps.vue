    ...
    methods: {
        ...
        genOpt: function() {
            ...
            if (size(vo.items) > 0 || !vo.firstLoading) {
                ...
                opt = {
                    ...
                    kpHeadFocusHighlight: { //此欄之儲存格不顯示焦點框(欄內為時間選擇器, 焦點框會與控制項本身之框線疊加); w-aggrid-vue 2.0.87 起補 :focus-within, 子控制項取得焦點時亦確實不顯示(該版前僅 :focus, 故效果不完全)
                        'timeBlocked': false,
                    },
                    rowsChange: (rs) => {
        ...
        saveIps: function() {
            ...
            async function core() {
                ...
                //updateIpsList (帶 lang, 後端 reject { key, msg } 之 msg 依 lang 翻譯)
                await vo.$fapi.updateIpsList(token, lang, rows)
                    .catch((err) => {
                        errTemp = err
                    })
