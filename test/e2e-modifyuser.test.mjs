    ...
    for (let lang of langs) {
        ...
        describe(`ModifyUser E2E [${lang}] — 後台變更使用者資訊`, function() {
            ...
            async function assertBaseline(buf, caseName) {
                ...
                assert.strict.equal(fs.existsSync(baselinePath), true, `標準圖不存在: ${baselinePath}, 請先執行 node test/e2e-modifyuser.test.mjs --baseline`)
                let baselineBuf = fs.readFileSync(baselinePath)
                //debug: assertion 前先存 test capture, 確保 fail 時有證據可 diff
                if (!fs.existsSync('./tmp')) fs.mkdirSync('./tmp', { recursive: true })
                fs.writeFileSync(`./tmp/modifyuser-${lang}-${caseName}-test.png`, buf)
                assert.strict.equal(buf.equals(baselineBuf), true, `截圖與標準圖不一致: modifyuser-${lang}-${caseName}`)
            }
