//【golden 真理來源 — 改造前 (git 5d341b8^) 之 server/staLogs/staUserAccountLogin.mjs 原樣留存, 只改 filterVpfsByWindow 之 import 路徑】
//用途: 由 gen-expected.mjs 於假時鐘下產出 expected-hr.json / expected-day.json; 不供 production 使用, 不要「修正」其語意 (含 ot(undefined) 視為 now 之舊行為)。
//注意: 原碼 import 為 'lodash/*.js' (非 lodash-es), 保留原樣; 本專案 node_modules/lodash 確認存在, 不需改為 lodash-es。
