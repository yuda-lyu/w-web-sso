...
> 本流程為「管理員幫使用者重設密碼」的補救機制，不同於 [z流程_使用者變更密碼.md](z流程_使用者變更密碼.md)（使用者自主變更，須知舊密碼+已登入）。
>
...
- 管理員對自己觸發重設, 後端拒絕（避免自我鎖死，且管理員自己改密碼應走「使用者變更密碼」流程）
- 一般使用者透過 API 直接呼叫觸發 reset, 後端拒絕（須 admin 權限）

### 二、使用者收信並登入

- 使用者收信, 信中含帳號 + 明文新密碼, 不含連結
- 使用者持新密碼登入（走「[使用者一般登入流程](z流程_使用者一般登入.md)」, 帳密通過驗證）
...
003  進入「[一般登入流程](z流程_使用者一般登入.md)」（procCore loginByAccountAndPassword 一路驗證）
...
| 比較項目 | 使用者變更密碼（已上線） | 使用者重設密碼（本文件 / 規劃中） |
|---|---|---|
| 入口 | user view 內「變更密碼」按鈕 | 管理員後台「重設密碼」按鈕 → 使用者收信後一般登入 |
| 操作者 | 使用者本人 | 管理員觸發, 使用者後續完成 |
| 須舊密碼 | 是（自己原本的） | 是（管理員給的隨機密碼） |
| 寄信時機 | 變更成功後通知 | 重設時寄出含明文新密碼，變更成功後再寄一封通知信 |
| token / 連結機制 | 無 | 無（不走連結） |
| 限時機制 | 無 | 無（依設計取消限時） |
| 帳號凍結 | 否 | 否（直接覆寫 password 即達效果，舊密碼自動失效） |
| 強制 UX | 無 | 是（isForceChangePw='y' 時登入後強制變更） |
| 錯誤呈現 | inline 紅字於對應欄下方 | 同左（重用同一個變更密碼介面與其錯誤呈現） |
