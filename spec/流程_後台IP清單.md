...
後端 `getIpsList` / `updateIpsList` 皆以 `funCheckAdmin` 包裝（`server/WWebSso.mjs:1254,1264`，funCheckAdmin 施加於 `checkTokenAndGetIpsList` / `checkTokenAndUpdateIpsList`，`server/procCore.mjs:1899,1922`），**僅具 admin 權限的有效 token 可存取**；token 無效／過期／非 admin 一律 reject。
...
009              checkTokenAndUpdateIpsList(token, rows, {fun: funCheckAdmin})  [procCore.mjs:1922]
010                  checkToken(token, opt)  [procCore.mjs:1925]
011                      token 無效 / 過期 / 非 admin:
012                          reject  → errTemp 被設值  [LayoutContentIps.vue:721]
013                  updateIpsList(rows)  [procCore.mjs:1928]
014                      updateTabItems('ips', rows, 'id', {resetOrder:false}) 差異更新  [procCore.mjs:1915]
